...
int main(int argc, char *argv[]) {
    ...
    if (fd < 0) {
        perror("打开输入设备失败");
        printf("请检查设备路径 /dev/input/event4 是否存在\n");
        printf("可能需要root权限运行\n");
        return -1;
    }
    ...
    if (argc == 1) {
        ...
        if (strcmp(argv[1], "simple") == 0 && argc >= 3) {
            int duration = atoi(argv[2]);
            simple_vibrate(fd, duration);
        } else if (strcmp(argv[1], "constant") == 0 && argc >= 4) {
            int duration = atoi(argv[2]);
            int strength = atoi(argv[3]);
            play_constant_effect(fd, duration, strength);
        } else if (strcmp(argv[1], "predefined") == 0 && argc >= 3) {
            int effect_id = atoi(argv[2]);
            int strength = (argc >= 4) ? atoi(argv[3]) : 0x7FFF;
            ...
            printf("用法:\n");
            printf("  %s                               # 运行测试序列\n", argv[0]);
            printf("  %s simple <duration_ms>          # 简单震动\n", argv[0]);
            printf("  %s constant <duration_ms> <strength> # 常量震动\n", argv[0]);
            printf("  %s predefined <effect_id> [strength] # 预定义效果\n", argv[0]);
            printf("  %s test                          # 运行测试\n", argv[0]);
            printf("\n参数说明:\n");
